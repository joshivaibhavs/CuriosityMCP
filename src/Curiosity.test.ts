...
describe('Curiosity', () => {
    ...
    describe('Tool Registration and Usage', () => {
        it('should register a tool', () => {
            const curiosity = new Curiosity(chatContainer);
            const tool = new ActionTool({ name: 'testTool', description: 'A test tool', exampleUsage: '', action: () => {} });
        ...
        it('should warn when registering a tool with a duplicate name', () => {
            const curiosity = new Curiosity(chatContainer);
            const tool1 = new ActionTool({ name: 'testTool', description: 'A test tool', exampleUsage: '', action: () => {} });
            const tool2 = new ActionTool({ name: 'testTool', description: 'Another test tool', exampleUsage: '', action: () => {} });
        ...
        it('should execute an ActionTool when called by the AI', async () => {
            ...
            const tool = new ActionTool({ name: 'doAction', description: 'Performs an action', exampleUsage: '', action: actionFn });
            const mockBackend = createMockAIBackend({
                'use the action tool': async function* () {
                    yield JSON.stringify({ tool: 'doAction', args: { payload: 'test' } });
                }
            });
        ...
        it('should execute a QueryTool and send the result back to the AI', async () => {
            ...
            const tool = new QueryTool({ name: 'doQuery', description: 'Performs a query', exampleUsage: '', query: queryFn });
            const mockBackend = createMockAIBackend({
                'use the query tool': async function* () {
                    yield JSON.stringify({ tool: 'doQuery', args: { query: 'info' } });
                }
            });
            ...
            expect(mockBackend.streamMessage).toHaveBeenLastCalledWith('Tool doQuery returned: "some data"', [tool]);
        });

        it('should display an error if a non-existent tool is called', async () => {
            const mockBackend = createMockAIBackend({
                'use bad tool': async function* () {
                    yield JSON.stringify({ tool: 'nonExistentTool', args: {} });
                }
            });
